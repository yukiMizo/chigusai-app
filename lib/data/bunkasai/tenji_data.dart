class TenjiDetailData {
  final String hr;
  final String title;
  final String pr;
  final String imgPath;

  const TenjiDetailData({
    required this.hr,
    required this.title,
    required this.pr,
    required this.imgPath,
  });
}

class TenjiData {
  static const List<TenjiDetailData> tenjiDataList = [
    TenjiDetailData(
      hr: "101",
      title: "かぐや様は被告人",
      pr: "今宵の被告人は殺人罪で起訴されたかぐや姫。\n貴公子は姫の美貌に殺されたのか？貴公子の愛は強愛？狂愛？\n悲劇が紐解かれていく中、判決を下すのはあなたです。",
      imgPath: "assets/images/bunkasai/tenji/101.jpg",
    ),
    TenjiDetailData(
      hr: "102",
      title: "プーさんのマニーハント",
      pr: "プーは困っていた。お金がない。\nこんなんじゃハチミツなんて食べれない‥\nそんなプーは今では悠々自適の生活。\nさあ、プーのマニーハントの流れをトロッコに乗って学ぼう。",
      imgPath: "assets/images/bunkasai/tenji/102.jpg",
    ),
    TenjiDetailData(
      hr: "103",
      title: "CINDERELLA~あなたは誰とマッチシンデレラ~",
      pr: "シンデレラの世界に迷い込んだあなた。しかしなんだか様子がおかしいです。時は中世、美しい娘に襲いかかる現代社会の恐怖とは！\nビビデバビデブーッ!103で待ってます!",
      imgPath: "assets/images/bunkasai/tenji/103.jpg",
    ),
    TenjiDetailData(
      hr: "104",
      title: "謎解きはJourney（長旅）のあとで",
      pr: "中館一階に、ちょっと不思議なレストランがオープンしました。ここのコース料理、まるでJourney。四つの謎に彩られた小さな世界旅行をあなたに。",
      imgPath: "assets/images/bunkasai/tenji/104.jpg",
    ),
    TenjiDetailData(
      hr: "105",
      title: "サバイバル・インポッシブル",
      pr: "もし無人島に流れ着いた時、生き残る自信はありますか?\n鍵となるのは、アイテムをどう使うか。年齢なんて関係ない!あなたのサバイバル生活がはじまります!",
      imgPath: "assets/images/bunkasai/tenji/105.jpg",
    ),
    TenjiDetailData(
      hr: "106",
      title: "STAR WALK",
      pr: "地球に住めなくなった後の世界...\n人間はどこで、どうやって生きていくのか、皆さんがまだ知らないそんな近い将来起こりうる未来を楽しく学べる展示です!!",
      imgPath: "assets/images/bunkasai/tenji/106.jpg",
    ),
    TenjiDetailData(
      hr: "107",
      title: "スマホゲットだぜ!",
      pr: "私たち107のテーマは「ネット依存」です!ポケモンの世界でサトシがスマホを手に入れたら...\nサトシが人生のドン底へ堕ちていく物語を是非お楽しみ下さい!!",
      imgPath: "assets/images/bunkasai/tenji/107.jpg",
    ),
    TenjiDetailData(
      hr: "108",
      title: "破産で誤算",
      pr: "破産はなぜ起こるのだろうか。\n事故、詐欺、ギャンブル依存...\n身近に破産の危険が潜んでいる。\nこれは、ある浮浪者の物語。",
      imgPath: "assets/images/bunkasai/tenji/108.jpg",
    ),
    TenjiDetailData(
      hr: "109",
      title: "SUPER KURIBO BROTHERS",
      pr: "これは、「スーパーマリオブラザーズ」のもうひとつの物語。\n敵の世界をクリボー視点で体験！\n109展示でスーパーマリオワールドを体験しよう！",
      imgPath: "assets/images/bunkasai/tenji/109.jpg",
    ),
    TenjiDetailData(
      hr: "201",
      title: "I AM・・・",
      pr: "世界中に派遣されるAIロボットを大量生産する工場で大事件が!\nいつか人間はAIに抜かされると言われているこの時代、あなたは自分を見失わずに生きていけますか?",
      imgPath: "assets/images/bunkasai/tenji/201.jpg",
    ),
    TenjiDetailData(
      hr: "202",
      title: "児童販売機",
      pr: "202特製のスペシャルドリンクで貴方のコンプレックスを無くしませんか？\nただし作り方は企業秘密、絶対に口外しないように...\nでは202でお待ちしております。",
      imgPath: "assets/images/bunkasai/tenji/202.jpg",
    ),
    TenjiDetailData(
      hr: "203",
      title: "DRAGON QUEST 20XX",
      pr: "時は20xx年-暴走するAIに侵食される町 を救うため冒険に出よう!\n我々人間とAIが 共存できる未来を救え!これは君が 主人公になれるゲームだ！",
      imgPath: "assets/images/bunkasai/tenji/203.jpg",
    ),
    TenjiDetailData(
      hr: "204",
      title: "ヌスミーマート",
      pr: "コンビニ「ヌスミーマート」での万引き体験をおこなっています！\nフォトスポットもあるので、思い出になることまちがいなし。\nぜひ204にお越しください！",
      imgPath: "assets/images/bunkasai/tenji/204.jpg",
    ),
    TenjiDetailData(
      hr: "205",
      title: "人材発掘オーディション",
      pr: "我が社では様々なジャンルで活躍できる新人のオーディションを開催しています。フレッシュな方々のご応募お待ちしております!\nたった１日で人生が変わるかも...",
      imgPath: "assets/images/bunkasai/tenji/205.jpg",
    ),
    TenjiDetailData(
      hr: "206",
      title: "Eyes~ MISSION×DARK SEE WITH~",
      pr: "宇宙船が故障して星に不時着？！ \n科学研究所に時限 爆弾が仕掛けられた？！ きみはあらゆる困難を乗り越えて無事にこの空間から脱出することはできるのか？！",
      imgPath: "assets/images/bunkasai/tenji/206.jpg",
    ),
    TenjiDetailData(
      hr: "207",
      title: "SPACE DEBRIS",
      pr: "西暦2223年の未来。人類の生活の場に宇宙が入りこんでいた。宇宙ゴミが深刻化した世界。君たちはこの危機を救えるか。",
      imgPath: "assets/images/bunkasai/tenji/207.jpg",
    ),
    TenjiDetailData(
      hr: "208",
      title: "あなたはキットスル",
      pr: "人気アイドルが訪れた館で事件は起きた。なぜ彼女は死ななければならなかったのか？\nこの体験を通して君の世界は180度変わるだろう。君にこの謎が解けるか。",
      imgPath: "assets/images/bunkasai/tenji/208.jpg",
    ),
    TenjiDetailData(
      hr: "209",
      title: "POACHER x POACHER",
      pr: "大正より蔓延り続ける自然界の闇、そう SECRET HUNTING 我々は解決困難なこの問題にどう立ち向かうのか。\n密猟はなぜ抹消されないのか。\nComing Boon",
      imgPath: "assets/images/bunkasai/tenji/209.jpg",
    ),
  ];
}
